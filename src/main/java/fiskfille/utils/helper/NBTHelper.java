...
public class NBTHelper {
    ...
    public static <T> T readFromNBT(NBTBase tag, ClassType<T> typeClass) {
        if (ISerializableObject.class.isAssignableFrom(typeClass.getType())) {
            if (ADAPTERS.containsKey(typeClass.getType())) {
                return (T) ADAPTERS.get(typeClass.getType())
                    .readFromNBT(tag);
            }
    ...
    public static <T extends FiskRegistryEntry<T>> List<T> readNBTList(NBTTagCompound compound, String name,
    ...
    public static <T extends ISerializableObject<T>> void registerAdapter(Class<? extends T> type,
