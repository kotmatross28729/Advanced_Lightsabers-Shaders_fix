...
public class VectorHelper {
    ...
    public static Vec3 getOffsetCoords(EntityLivingBase entity, double xOffset, double yOffset, double zOffset,
        ...
        vec3.rotateAroundX(
            -(entity.rotationPitch + (entity.rotationPitch - entity.prevRotationPitch) * partialTicks) * (float) Math.PI
                / 180.0F);
        vec3.rotateAroundY(
            -(entity.rotationYaw + (entity.rotationYaw - entity.prevRotationYaw) * partialTicks) * (float) Math.PI
                / 180.0F);

        return Vec3.createVectorHelper(
            entity.posX + vec3.xCoord,
            entity.posY + getOffset(entity) + vec3.yCoord,
    ...
    public static Vec3 add(Vec3 vec3, Vec3 vec31) {
        return Vec3
            .createVectorHelper(vec3.xCoord + vec31.xCoord, vec3.yCoord + vec31.yCoord, vec3.zCoord + vec31.zCoord);
    }
    ...
    public static Vec3 getBackSideCoordsRenderYawOffset(EntityLivingBase entity, double amount, boolean side,
        double backAmount, boolean pitch) {
        Vec3 front = getFrontCoordsRenderYawOffset(entity, backAmount, pitch)
            .addVector(-entity.posX, -(entity.posY + getOffset(entity)), -entity.posZ);
    ...
    public static Vec3 getSideCoordsRenderYawOffset(EntityLivingBase entity, double amount, int side) {
        ...
        float xScale = f4 * f5;
        float zScale = f3 * f5;
        return Vec3.createVectorHelper(entity.posX, entity.posY + getOffset(entity), entity.posZ)
            .addVector(xScale * amount, yScale * amount, zScale * amount);
    }
    ...
    public static <T extends Entity> List<T> getEntitiesNear(Class<T> type, World world, double x, double y, double z,
        double radius) {
