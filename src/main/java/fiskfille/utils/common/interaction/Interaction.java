...
public abstract class Interaction extends FiskRegistryEntry<Interaction> {
    ...
    public static final FiskRegistryNamespaced<Interaction> REGISTRY = new FiskRegistryNamespaced(FiskUtils.MODID, null)
        .setMaxId(MAX_ID);
    ...
    public abstract boolean listen(EntityPlayer sender, EntityPlayer clientPlayer, InteractionType type, Side side,
        int x, int y, int z, List<Integer> args);

    public abstract void receive(EntityPlayer sender, EntityPlayer clientPlayer, InteractionType type, Side side, int x,
        int y, int z, Integer[] args);

    public boolean listen(EntityPlayer sender, EntityPlayer clientPlayer, InteractionType type, Side side, int x, int y,
        int z) {
