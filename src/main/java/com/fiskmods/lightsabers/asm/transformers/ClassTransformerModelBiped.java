...
public class ClassTransformerModelBiped extends ClassTransformerBase {
    ...
    public boolean processMethods(List<MethodNode> methods) {
        ...
        for (MethodNode method : methods) {
            if (method.name.equals(getMappedName("a", "render"))
                && method.desc.equals(getMappedName("(Lsa;FFFFFF)V", "(Lnet/minecraft/entity/Entity;FFFFFF)V"))) {
                ...
                for (int i = 0; i < method.instructions.size(); ++i) {
                    ...
                    if (node instanceof MethodInsnNode) {
                        ...
                        if (methodNode.name.equals(getMappedName("a", "setRotationAngles")) && methodNode.desc
                            .equals(getMappedName("(FFFFFFLsa;)V", "(FFFFFFLnet/minecraft/entity/Entity;)V"))) {
                            list.add(node);
                ...
                flag = true;
            } else if (method.name.equals(getMappedName("a", "setRotationAngles"))
                && method.desc.equals(getMappedName("(FFFFFFLsa;)V", "(FFFFFFLnet/minecraft/entity/Entity;)V"))) {
                    InsnList list = new InsnList();
                    ...
                    for (int i = 0; i < method.instructions.size(); ++i) {
                        ...
                        // list.add(new MethodInsnNode(INVOKESTATIC, Type.getInternalName(ModelHelper.class),
                        // "setRotationAngles", getMappedName("(Lbhm;FFFFFFFLsa;)V",
