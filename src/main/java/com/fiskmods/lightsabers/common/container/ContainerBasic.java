...
public class ContainerBasic<T extends TileEntity> extends Container {
    ...
    public void addPlayerInventory(InventoryPlayer inventoryPlayer, int yOffset) {
        ...
        for (i = 0; i < 3; ++i) {
            for (j = 0; j < 9; ++j) {
                addSlotToContainer(
                    makeInventorySlot(inventoryPlayer, j + i * 9 + 9, 8 + j * 18, 84 + yOffset + i * 18));
            }
        }
    ...
    public boolean canInteractWith(EntityPlayer player) {
        if (tileentity != null) {
            ...
            return player.getDistanceSq(tileentity.xCoord + 0.5D, tileentity.yCoord + 0.5D, tileentity.zCoord + 0.5D)
                <= 64.0D;
        }
        ...
        if (stackToMove.isStackable()) {
            while (stackToMove.stackSize > 0 && (!descending && id < toId || descending && id >= fromId)) {
                ...
                if ((!check || slot.isItemValid(stackToMove)) && dstStack != null
                    && dstStack.getItem() == stackToMove.getItem()
                    && (!stackToMove.getHasSubtypes() || stackToMove.getItemDamage() == dstStack.getItemDamage())
                    && ItemStack.areItemStackTagsEqual(stackToMove, dstStack)) {
                    int maxStackSize = Math.min(
                        slot.inventory.getInventoryStackLimit(),
                        Math.min(dstStack.getMaxStackSize(), slot.getSlotStackLimit()));
        ...
        if (stackToMove.stackSize > 0) {
            ...
            while (!descending && id < toId || descending && id >= fromId) {
                ...
                if ((!check || slot.isItemValid(stackToMove)) && dstStack == null) {
                    int maxStackSize = Math.min(
                        slot.inventory.getInventoryStackLimit(),
                        Math.min(stackToMove.getMaxStackSize(), slot.getSlotStackLimit()));
