...
public class ALDataInterp<T> extends ALData<T> {
    ...
    public T interpolate(Entity entity, float progress) {
        if (progress == 1) {
            ...
            return (T) Float
                .valueOf(FiskServerUtils.interpolate((Float) getPrev(entity), (Float) get(entity), progress));
        } else if (ofType(Double.class)) {
            return (T) Double
                .valueOf(FiskServerUtils.interpolate((Double) getPrev(entity), (Double) get(entity), progress));
        } else {
