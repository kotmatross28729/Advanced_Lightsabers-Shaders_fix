...
public class StatusEffect implements Comparable<StatusEffect> {
    ...
    public static StatusEffect readFromNBT(NBTTagCompound tag) {
        ...
        if (effect != null) {
            ...
            if (tag.hasKey("CasterUUIDMost", NBT.TAG_ANY_NUMERIC)
                && tag.hasKey("CasterUUIDLeast", NBT.TAG_ANY_NUMERIC)) {
    ...
    public static void add(EntityLivingBase entity, EntityLivingBase caster, Effect effect, int duration,
        int amplifier) {
                ...
                if (prevDuration != status.duration || prevAmplifier != status.amplifier) {
                    if (!entity.worldObj.isRemote) {
                        ALNetworkManager.wrapper
                            .sendToDimension(new MessageUpdateEffects(entity, list), entity.dimension);
                    }
    ...
    public static List<EntityLivingBase> getTargets(EntityLivingBase caster, Effect effect, int amplifier) {
        ...
        for (EntityLivingBase entity : Iterables.filter(caster.worldObj.loadedEntityList, EntityLivingBase.class)) {
            ...
            if (status != null && (amplifier < 0 || amplifier == status.amplifier)
                && status.casterUUID != null
                && status.casterUUID.equals(caster.getUniqueID())) {
                targets.add(entity);
            }
        }
