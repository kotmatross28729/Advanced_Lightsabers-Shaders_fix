...
public class ALData<T> extends FiskRegistryEntry<ALData<?>> {
    ...
    public static final FiskRegistryNamespaced<ALData<?>> REGISTRY = new FiskRegistryNamespaced(
        Lightsabers.MODID,
        null);
    ...
    public static final ALData<List<Power>> SELECTED_POWERS = new ALData<List<Power>>(Power.factory())
    ...
    private boolean setWithBypass(Entity entity, T value) {
        if (validUpdate(entity, value)) {
            if (entity instanceof IDataHolder) {
                ...
                ALPlayerData.getData((EntityPlayer) entity)
                    .putData(this, value);
                onValueChanged(entity, value);
            }
    ...
    public boolean clampWithoutNotify(Entity entity, T min, T max) {
        if (min instanceof Byte) {
            return setWithoutNotify(
                entity,
                (T) Byte.valueOf((byte) MathHelper.clamp_int((Byte) get(entity), (Byte) min, (Byte) max)));
        } else if (min instanceof Short) {
            return setWithoutNotify(
                entity,
                (T) Short.valueOf((short) MathHelper.clamp_int((Short) get(entity), (Short) min, (Short) max)));
        } else if (min instanceof Integer) {
            return setWithoutNotify(
                entity,
                (T) Integer.valueOf(MathHelper.clamp_int((Integer) get(entity), (Integer) min, (Integer) max)));
        } else if (min instanceof Long) {
            long l = (Long) get(entity);
            return setWithoutNotify(
                entity,
                (T) Long.valueOf(l < (Long) min ? (Long) min : l > (Long) max ? (Long) max : l));
        } else if (min instanceof Float) {
            return setWithoutNotify(
                entity,
                (T) Float.valueOf(MathHelper.clamp_float((Float) get(entity), (Float) min, (Float) max)));
        } else if (min instanceof Double) {
            return setWithoutNotify(
                entity,
                (T) Double.valueOf(MathHelper.clamp_double((Double) get(entity), (Double) min, (Double) max)));
        }
    ...
    public T get(Entity entity) {
        if (entity instanceof IDataHolder) {
            ...
            return ALPlayerData.getData((EntityPlayer) entity)
                .getData(this);
        }
        ...
        if (hasPerms(player.worldObj.isRemote ? Side.CLIENT : Side.SERVER) && legalUpdate(player)) {
            if (player.worldObj.isRemote) {
                ...
                ALNetworkManager.wrapper
                    .sendToDimension(new MessagePlayerData(player, this, get(player)), player.dimension);
            }
    ...
    public static Map<ALData, Object> fromBytes(ByteBuf buf, Map<ALData, Object> data) {
        ...
        if (size != ALData.REGISTRY.getKeys()
            .size()) {
            throw new RuntimeException(
                String.format(
                    "Incompatible data registries - this is really bad! Received %s, expected: %s",
                    size,
                    ALData.REGISTRY.getKeys()
                        .size()));
        }
