                ...
                if (!list.isEmpty()) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        worldObj.playAuxSFX(
                            ...
                            yCoord + i,
                            zCoord,
                            Block.getIdFromBlock(worldObj.getBlock(xCoord, yCoord + i, zCoord))
                                + (worldObj.getBlockMetadata(xCoord, yCoord + i, zCoord) << 12));
                    }

                    if (!worldObj.isRemote) {
                        BlockSithStoneCoffin.spawnSithGhost(worldObj, xCoord, yCoord, zCoord)
                            .setAttackTarget(list.get(0));
                    }
                }
            }
        } else {
