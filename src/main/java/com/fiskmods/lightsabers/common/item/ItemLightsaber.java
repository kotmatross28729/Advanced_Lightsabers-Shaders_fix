...
public class ItemLightsaber extends ItemLightsaberBase implements IBattlegearWeapon {
    ...
    public WeightedRandomChestContent getChestGenBase(ChestGenHooks chest, Random rand,
        ...
        if (itemstack.hasTagCompound() && itemstack.getTagCompound()
            .getBoolean("SithTombLoot")) {
            itemstack = LightsaberData
                .createRandom(rand, rand.nextInt(5) == 0 ? CrystalColor.PURPLE : CrystalColor.RED);

            if (rand.nextInt(4) == 0) {
                itemstack = ItemDoubleLightsaber.create(itemstack, itemstack);
            }
        } else if (itemstack.hasTagCompound() && itemstack.getTagCompound()
            .getBoolean("JediTempleLoot")) {
                itemstack = LightsaberData
                    .createRandom(rand, rand.nextBoolean() ? CrystalColor.MEDIUM_BLUE : CrystalColor.GREEN);
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int slot, boolean inHand) {
        if (!world.isRemote) {
            if (itemstack.hasTagCompound() && itemstack.getTagCompound()
                .hasKey("Lightsaber", NBT.TAG_COMPOUND)) {
