...
public abstract class ItemLightsaberBase extends ItemSword {
    ...
    public boolean onEntitySwing(EntityLivingBase entity, ItemStack itemstack) {
        if (isActive(itemstack)) {
            ...
            if (mop == null || mop.typeOfHit != MovingObjectType.BLOCK) {
                entity.playSound(
                    entity instanceof EntityPlayer ? ALSounds.player_lightsaber_swing : ALSounds.mob_lightsaber_swing,
                    1.0F,
                    1.0F);
            } else {
    ...
    private MovingObjectPosition getMovingObjectPosition(World world, EntityLivingBase entity, boolean b) {
        if (entity instanceof EntityPlayer) {
            ...
            float expand = 1.0F;
            List possibleEntities = world.getEntitiesWithinAABBExcludingEntity(
                entity,
                entity.boundingBox.addCoord(look.xCoord * reach, look.yCoord * reach, look.zCoord * reach)
            ...
            for (Object possibleEntity : possibleEntities) {
                ...
                if (selectingEntity.canBeCollidedWith()) {
                    ...
                    AxisAlignedBB selectionBounds = selectingEntity.boundingBox
                    ...
                    if (selectionBounds.isVecInside(position)) {
                        ...
                        if (distanceToEntity < closestDistance || closestDistance == 0.0D) {
                            if (selectingEntity == selectingEntity.ridingEntity
                                && !selectingEntity.canRiderInteract()) {
