...
public class ItemCrystal extends ItemBlock implements ILightsaberComponent {
    ...
    public WeightedRandomChestContent getChestGenBase(ChestGenHooks chest, Random rand,
        ...
        if (itemstack.hasTagCompound()) {
            category = itemstack.getTagCompound()
                .getString("ChestGenCategory");
        }

        List<CrystalColor> list = Lists.newArrayList();

        for (CrystalColor color : CrystalColor.values()) {
            if (Arrays.asList(chestMap.get(color))
                .contains(category)) {
                for (int i = rarityMap.get(color)
                    .ordinal() - 1; i >= 0; --i) {
                    list.add(color);
                }
            }
        ...
        return new WeightedRandomChestContent(
            create(list.get(rand.nextInt(list.size()))),
    ...
    public void onUpdate(ItemStack itemstack, World world, Entity entity, int slot, boolean inHand) {
        if (!world.isRemote) {
            if (itemstack.hasTagCompound() && itemstack.getTagCompound()
                .hasKey("color", NBT.TAG_ANY_NUMERIC)) {
    ...
    private static void init() throws Exception {
        ...
        registerRarity(
            DEEP_BLUE,
            common,
            ModChestGen.SITH_TOMB_COFFIN,
            ModChestGen.SITH_TOMB_TREASURY,
            ModChestGen.SITH_TOMB_ANNEX,
