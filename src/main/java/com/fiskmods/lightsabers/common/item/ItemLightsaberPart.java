...
public class ItemLightsaberPart extends Item implements ILightsaberComponent {
    ...
    @Override
    public WeightedRandomChestContent getChestGenBase(ChestGenHooks chest, Random rand,
        WeightedRandomChestContent original) {
        return new WeightedRandomChestContent(
            create(getRandomType(rand), Hilt.REGISTRY.getRandom(rand)),
    ...
    public static Hilt get(ItemStack itemstack) {
        ...
        if (itemstack.hasTagCompound()) {
            if (itemstack.getTagCompound()
                .hasKey("lightsaber", NBT.TAG_STRING)) {
                id = Hilt.REGISTRY.getIDForObject(
                    Hilt.REGISTRY.getObject(
                        Hilt.LEGACY_MAPPINGS.get(
                            itemstack.getTagCompound()
                                .getString("lightsaber"))));
                itemstack.getTagCompound()
                    .removeTag("lightsaber");
                itemstack.getTagCompound()
                    .removeTag("type");
                itemstack.getTagCompound()
                    .setByte(ALConstants.TAG_PART, (byte) id);
            } else if (itemstack.getTagCompound()
                .hasKey(ALConstants.TAG_PART, NBT.TAG_ANY_NUMERIC)) {
                    id = itemstack.getTagCompound()
                        .getByte(ALConstants.TAG_PART);
                }
        }
    ...
    public static ItemStack create(PartType type, Hilt hilt) {
        ...
        itemstack.getTagCompound()
            .setByte(ALConstants.TAG_PART, (byte) Hilt.REGISTRY.getIDForObject(hilt));

        return itemstack;
    }

    public static PartType getType(ItemStack itemstack) {
        return itemstack.getItem() instanceof ItemLightsaberPart ? ((ItemLightsaberPart) itemstack.getItem()).partType
            : null;
    }
