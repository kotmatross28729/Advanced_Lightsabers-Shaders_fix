...
public class PowerEffectChoke extends PowerEffect {
    ...
    public boolean execute(EntityPlayer player, Side side) {
        ...
        if (rayTrace != null && rayTrace.typeOfHit == MovingObjectType.ENTITY
    ...
    public String[] getDesc() {
        return new String[] {
            PowerDesc.create(
                "effect",
                PowerDesc.format("%s %s%s", 2 + amplifier * 2, Unit.DAMAGE, EnumChatFormatting.GRAY + "/"),
                Target.TARGET),
            PowerDesc.create(
                "effect",
                PowerDesc.format("%s %s%s", Effect.STUN, EnumChatFormatting.GRAY, getStunDuration(amplifier)),
                Target.TARGET) };
    }
