...
public class PowerEffectHeal extends PowerEffect {
    ...
    public boolean execute(EntityPlayer player, Side side) {
        ...
        if (aoeHeal > 0) {
            AxisAlignedBB aabb = player.boundingBox.copy()
                .expand(6, 6, 6);
    ...
    public void doParticles(EntityLivingBase entity) {
        for (int i = 0; i < 16; ++i) {
            ALParticles.spawnParticle(
                ALParticleType.FORCE_HEAL,
                entity.posX + (rand.nextFloat() * 2 - 1) * entity.width,
                entity.boundingBox.minY + entity.height / 3 + (entity.height / 3) * 2 * rand.nextFloat(),
                entity.posZ + (rand.nextFloat() * 2 - 1) * entity.width,
                0,
                0,
                0);
        }
