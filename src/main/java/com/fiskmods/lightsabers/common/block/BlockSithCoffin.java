...
public class BlockSithCoffin extends BlockContainer implements ITileEntityProvider {
    ...
    public void sendActionPacket(TileEntitySithCoffin tile, EntityPlayer player, int action) {
        if (player.worldObj.isRemote) {
            ALNetworkManager.wrapper
                .sendToServer(new PacketTileAction(player, tile.xCoord, tile.yCoord, tile.zCoord, action));
        }
    }
    ...
    @Override
    public void dropBlockAsItemWithChance(World world, int x, int y, int z, int metadata, float dropChance,
        int fortune) {
        if (!isBlockFrontOfCoffin(metadata)) {
