...
public class BlockCrystalGen extends BlockBasic {
    ...
    public static void replace(World world, int x, int y, int z) {
        if (ModBlocks.lightsaberCrystal.canPlaceBlockAt(world, x, y, z)
            && world.setBlock(x, y, z, ModBlocks.lightsaberCrystal, 0, 2)) {
            TileEntityCrystal tile = (TileEntityCrystal) world.getTileEntity(x, y, z);

            if (tile != null) {
                Random rand = new Random(
                    world.getSeed() + x * x * 0x4c1906 + x * 0x5ac0db + z * z * 0x4307a7L + z * 0x5f24f ^ 0x3ad8025f
                        ^ y);
