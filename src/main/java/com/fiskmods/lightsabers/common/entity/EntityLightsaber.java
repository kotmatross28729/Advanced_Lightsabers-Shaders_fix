...
public class EntityLightsaber extends EntityThrowable implements IEntityAdditionalSpawnData {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (ticksExisted % 3 == 0) {
                worldObj.playSoundAtEntity(
                    this,
                    thrower instanceof EntityPlayer ? ALSounds.player_lightsaber_swing : ALSounds.mob_lightsaber_swing,
                    1,
                    1);
            }
            ...
            if (thrower != null && getShouldReturn()) {
                if (getDistanceToEntity(thrower) <= 2) {
                    ...
                    Vec3 vec3 = Vec3
                        .createVectorHelper(
                            thrower.posX - posX,
                            thrower.posY - posY + thrower.height * 0.6,
                            thrower.posZ - posZ)
                        .normalize();
                    double d = 2;
        ...
        if (!getShouldReturn()) {
            ...
            for (rotationPitch = (float) (Math.atan2(motionY, motion) * 180 / Math.PI); rotationPitch
                - prevRotationPitch < -180; prevRotationPitch -= 360) {
                ;
            }
    ...
    protected void onImpact(MovingObjectPosition mop) {
        ...
        if (thrower != null) {
            ...
            if (mop.entityHit != null && mop.entityHit != thrower) {
                if (lightsaber != null) {
                    Collection<AttributeModifier> c = lightsaber.getAttributeModifiers()
