...
public class ClientEventHandler {
    ...
    public void onLivingUpdate(LivingUpdateEvent event) {
        ...
        // {
        // if (itemstack != null && itemstack1 != null && itemstack.getItem() instanceof ItemLightsaberBase &&
        // itemstack1.getItem() instanceof ItemLightsaberBase && ItemLightsaberBase.isActive(itemstack) !=
        // ItemLightsaberBase.isActive(itemstack1) || itemstack != null && itemstack1 == null && itemstack.getItem()
        // instanceof ItemLightsaberBase || itemstack == null && itemstack1 != null && itemstack1.getItem() instanceof
        // ItemLightsaberBase)
        // {
            ...
            if (player == mc.thePlayer) {
                ...
                for (int i = 0; i < inventory.mainInventory.length; ++i) {
                    if (inventory.mainInventory[i] != null
                        && inventory.mainInventory[i].getItem() == ModItems.lightsaber) {
                ...
                if (power != null && ALData.USE_POWER_COOLDOWN.get(player) == 0
                ...
                if (!flag && mc.entityRenderer.getShaderGroup() != null) {
                    String s = mc.entityRenderer.getShaderGroup()
                        .getShaderGroupName();
                    ResourceLocation[] shaders = { ALRenderHelper.SHADER_BLUE, ALRenderHelper.SHADER_GRAY,
                        ALRenderHelper.SHADER_BLUR };
    ...
    public void onRenderPlayerSpecialsPre(RenderPlayerEvent.Specials.Pre event) {
        ...
        if (data != null && !data.equals(LightsaberData.EMPTY)
            && (player.getHeldItem() == null || !player.getHeldItem()
                .getItem()
                .equals(ModItems.lightsaber))) {
            GL11.glPushMatrix();
            ...
            GL11.glTranslatef(
                0,
                -(data.getPart(PartType.BODY).height + data.getPart(PartType.POMMEL).height / 2) * 0.0625F,
                0);
                ...
                for (int j = -amount; j <= amount; ++j) {
                    for (int k = -amount; k <= amount; ++k) {
                        ...
                        tessellator.setColorRGBA(
                            ...
                            84,
                            181,
                            (int) (50 * Math.min((effectStun.duration - renderTick) / 20, 1)));
    ...
    public void onRenderLivingPre(RenderLivingEvent.Pre event) {
        ...
        if (StatusEffect.get(entity, Effect.STEALTH) != null
            && (effectGaze == null || !ALRenderHelper.canGazeEntity(mc.thePlayer, entity, effectGaze.amplifier))) {
        ...
        if (entity != mc.thePlayer && entity != mc.thePlayer.ridingEntity
            && effectGaze != null
            && ALRenderHelper.canGazeEntity(mc.thePlayer, entity, effectGaze.amplifier)) {
            if (entity.stepHeight > 0) {
                ...
                if (!flag1) {
                    RenderManager.instance.renderEntityWithPosYaw(
                        entity,
                        event.x,
                        event.y + (entity == mc.thePlayer ? entity.yOffset : 0),
                        event.z,
                        f5,
                        renderTick);
                    flag1 = true;
    ...
    public void onSound(PlaySoundEvent17 event) {
        if (mc.thePlayer != null) {
            ...
            if (!name.equals(ALSounds.player_force_stealth_on) && !name.equals(ALSounds.player_force_stealth_off)
                && !name.startsWith(ALSounds.ambient_stealth)
                && StatusEffect.get(mc.thePlayer, Effect.STEALTH) != null) {
                ISound sound = event.sound;
                event.result = new SoundAL(
                    sound.getPositionedSoundLocation(),
                    sound.getVolume() * 0.05F,
                    sound.getPitch(),
                    sound.canRepeat(),
