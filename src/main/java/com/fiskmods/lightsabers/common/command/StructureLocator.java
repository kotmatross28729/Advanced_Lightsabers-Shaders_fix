...
public class StructureLocator implements Runnable {
    ...
    public StructureLocator(CommandBase commandBase, ICommandSender commandSender, EnumStructure enumStructure,
        boolean all, int range, int x, int z) {
        command = commandBase;
    ...
    public void run() {
            ...
            for (int i = -range; i <= range; ++i) {
                for (int j = -range; j <= range; ++j) {
                    ...
                    for (EnumStructure structure1 : EnumStructure.values()) {
                        if (WorldGeneratorStructures.canSpawnStructureAtCoords(
                            world,
                            chunk.getCenterXPos(),
        ...
        Comparator<ChunkCoordIntPair> coordComparator = new Comparator<ChunkCoordIntPair>() {
            ...
            public int compare(ChunkCoordIntPair arg0, ChunkCoordIntPair arg1) {
                return Double.valueOf(
                    Vec3.createVectorHelper(arg0.chunkXPos, 0, arg0.chunkZPos)
                        .distanceTo(vec))
                    .compareTo(
                        Vec3.createVectorHelper(arg1.chunkXPos, 0, arg1.chunkZPos)
                            .distanceTo(vec));
            }
        };
        ...
        if (targetAll) {
            ...
            if (structures.isEmpty()) {
                ...
                for (EnumStructure structure1 : EnumStructure.values()) {
                    if (!getStructures(world, structure1).isEmpty() && structures.get(0)
                        .equals(getStructures(world, structure1).get(0))) {
                        ChunkCoordIntPair chunk = structures.get(0);
                        CommandBase.func_152373_a(
                            sender,
                            command,
                            "commands.force.structure.locate.success",
                            ALHelper.getUnconventionalName(structure1.name()),
                            chunk.getCenterXPos(),
                            chunk.getCenterZPosition());
                        break;
                    }
