...
public abstract class Structure {
    ...
    private void setBlock(int x, int y, int z, Block block, int metadata) {
        ...
        if (simulate
            || ((block1 = worldObj.getBlock(x, y, z)) != block || worldObj.getBlockMetadata(x, y, z) != metadata)
                && block1.getBlockHardness(worldObj, x, y, z) != -1) {
            if (simulate) {
    ...
    protected boolean generateStructureChestContents(Random random, int x, int y, int z,
        WeightedRandomChestContent[] chestContent, int itemsToGenerate) {
        int i = xCoord + x;
        int j = yCoord + y;
        int k = zCoord + z;
    ...
    protected boolean fillStructureInventory(Block block, Random random, int x, int y, int z,
        WeightedRandomChestContent[] chestContent, int itemsToGenerate) {
        int i = xCoord + x;
        int j = yCoord + y;
        int k = zCoord + z;
