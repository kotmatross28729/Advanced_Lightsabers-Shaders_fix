...
public class GuiHandlerAL implements IGuiHandler {
    ...
    public Object getServerGuiElement(int id, EntityPlayer player, World world, int x, int y, int z) {
        ...
        switch (id) {
            case 0:
                return world.getBlock(x, y, z) instanceof BlockLightsaberForge
                    ? new ContainerLightsaberForge(player.inventory, (TileEntityLightsaberForge) tile)
                    : null;
            case 1:
                return world.getBlock(x, y, z) == ModBlocks.sithCoffin
                    ? new ContainerSithCoffin(player.inventory, (TileEntitySithCoffin) tile)
                    : null;
            case 3:
                return new ContainerCrystalPouch(player.inventory, new InventoryCrystalPouch(player, x));
            case 4:
                return tile instanceof TileEntityDisassemblyStation
                    ? new ContainerDisassemblyStation(player.inventory, (TileEntityDisassemblyStation) tile)
                    : null;
        }
    ...
    public Object getClientGuiElement(int id, EntityPlayer player, World world, int x, int y, int z) {
        ...
        switch (id) {
            case 0:
                return world.getBlock(x, y, z) instanceof BlockLightsaberForge
                    ? new GuiLightsaberForge(player.inventory, (TileEntityLightsaberForge) tile)
                    : null;
            case 1:
                return world.getBlock(x, y, z) == ModBlocks.sithCoffin
                    ? new GuiSithCoffin(player.inventory, (TileEntitySithCoffin) tile)
                    : null;
            case 2:
                return world.getBlock(x, y, z) == ModBlocks.holocron
                    ? new GuiForcePowers(null, player, (TileEntityHolocron) tile)
                    : null;
            case 3:
                return new GuiCrystalPouch(player.inventory, new InventoryCrystalPouch(player, x));
            case 4:
                return tile instanceof TileEntityDisassemblyStation
                    ? new GuiDisassemblyStation(player.inventory, (TileEntityDisassemblyStation) tile)
                    : null;
        }
