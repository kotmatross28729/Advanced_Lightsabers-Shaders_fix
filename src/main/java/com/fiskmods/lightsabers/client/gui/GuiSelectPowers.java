...
public class GuiSelectPowers extends GuiScreen {
    ...
    public void initGui() {
        super.initGui();
        slots.clear();
        selectedPowers = ALData.SELECTED_POWERS.get(mc.thePlayer)
            .toArray(selectedPowers);
    ...
    protected void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        if (button == 0 && isShiftKeyDown()) {
            ...
            if (grabbedId < 0) {
                for (int i = 0; i < 3; ++i) {
                    if (selectedPowers[i] != null
                        && new Rectangle(x + 62 + i * 18, y + 142, 16, 16).contains(mouseX, mouseY)) {
        ...
        if (hoverPower != null) {
            ...
            if (hoverPower.hasIcon()) {
                ...
                ItemRenderer.renderItemIn2D(
                    tessellator,
                    hoverPower.getIconX() * 16 * f,
                    hoverPower.getIconY() * 16 * f,
                    (hoverPower.getIconX() * 16 + 16) * f,
                    (hoverPower.getIconY() * 16 + 16) * f,
                    16,
                    16,
                    0);
            ...
            drawGradientRect(
                ...
                x1 + fieldWidth,
                y1 + Math
                    .max((fontRendererObj.FONT_HEIGHT + 2) * list.size() + fontRendererObj.FONT_HEIGHT * 2 + 12, 52),
                0xA5222222,
                0xA5222222);
            ...
            if (stats.useCost > 0) {
                fontRendererObj.drawStringWithShadow(
                    I18n.format(
                        stats.powerType == PowerType.PER_USE ? "forcepower.perUse" : "forcepower.perSecond",
