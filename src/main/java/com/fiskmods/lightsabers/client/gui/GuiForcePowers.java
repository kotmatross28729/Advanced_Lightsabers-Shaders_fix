...
public class GuiForcePowers extends GuiScreen {
    ...
    public void onGuiClosed() {
        if (tile != null) {
            ALNetworkManager.wrapper
                .sendToServer(new PacketTileAction(mc.thePlayer, tile.xCoord, tile.yCoord, tile.zCoord, 1));
        }
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        if (Mouse.isButtonDown(0)) {
            ...
            if ((field_146554_D == 0 || field_146554_D == 1) && mouseX >= i1
                && mouseX < i1 + 224
                && mouseY >= j1
                && mouseY < j1 + 155) {
                if (field_146554_D == 0) {
    ...
    protected void drawForeground() {
        ...
        drawOutlinedString(
            StatCollector.translateToLocalFormatted(
                "gui.forcePowers.xp",
                MathHelper.floor_float(ALData.FORCE_XP.get(mc.thePlayer))),
            x,
            y,
            MathHelper.floor_float(ALData.FORCE_XP.get(mc.thePlayer)) > 0 ? 8453920 : 0xD74848);
        drawOutlinedString(
            StatCollector.translateToLocalFormatted("gui.forcePowers.basePower", ALData.BASE_POWER.get(mc.thePlayer)),
            x,
            y + 10,
            ALData.BASE_POWER.get(mc.thePlayer) > 0 ? 8453920 : 0xD74848);
    }
    ...
    protected void drawBackground(int mouseX, int mouseY, float partialTicks) {
        ...
        if (power != null) {
            ...
            if (!powerManager.canUnlockPower(power)) {
                ...
                if (hierarchy == 3) {
                    ...
                    s = new ChatComponentTranslation(
                        ...
                        new Object[] { power.parent.getLocalizedName() }).getUnformattedText();
                ...
                if (power.getActualXpCost(mc.thePlayer) != 0) {
                    desc.add(
                        (!powerManager.hasPowerUnlocked(power) ? EnumChatFormatting.RED : "")
                            + I18n.format("forcepower.cost", power.getActualXpCost(mc.thePlayer)));
                }

                if (stats.baseRequirement != 0) {
                    desc.add(
                        (!powerManager.hasPowerUnlocked(power)
                            && ALData.BASE_POWER.get(mc.thePlayer) < stats.baseRequirement ? EnumChatFormatting.RED
                                : "")
                            + I18n.format("forcepower.basePowerReq", stats.baseRequirement));
                }

                if (stats.useCost != 0) {
                    desc.add(
                        I18n.format(
                            stats.powerType == PowerType.PER_USE ? "forcepower.perUse"
                                : stats.powerType == PowerType.PER_SECOND ? "forcepower.perSecond"
                                    : "forcepower.passive",
                            ItemStack.field_111284_a.format(stats.useCost)));
                }
                ...
                if (stats.forceBonus != 0) {
                    desc.add(
                        I18n.format("forcepower.forcePower", (stats.forceBonus < 0 ? "-" : "+") + stats.forceBonus));
                }
                ...
                if (stats.regen != 0) {
                    desc.add(
                        I18n.format(
                            ...
                            (stats.regen < 0 ? "-" : "+") + stats.regen + (stats.regenOperation == 1 ? "%" : "")));
                }
                ...
                drawGradientRect(
                    ...
                    hoverY - 3,
                    hoverX + maxWidth + 3,
                    hoverY + 6 + (2 + fontRendererObj.FONT_HEIGHT) * (desc.size() + 1) + 12,
                    -1073741824,
                    -1073741824);
                ...
                if (powerManager.hasPowerUnlocked(power)) {
                    fontRendererObj
                        .drawStringWithShadow(I18n.format("forcepower.unlocked"), hoverX, height + 3, -7302913);
                } else {
            ...
            if (title != null) {
                fontRendererObj
                    .drawStringWithShadow(title, hoverX, hoverY, powerManager.canUnlockPower(power) ? -1 : -8355712);
            }

            if (Mouse.isButtonDown(0) && !powerManager.hasPowerUnlocked(power)
                && powerManager.canUnlockPower(power)
                && (MathHelper.floor_double(ALData.FORCE_XP.get(mc.thePlayer)) > 0
                    || power.getActualXpCost(mc.thePlayer) == 0)
                && (ALData.BASE_POWER.get(mc.thePlayer) >= power.powerStats.baseRequirement
